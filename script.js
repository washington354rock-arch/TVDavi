...
function mostrarStatusAdmin(mensagem, tipo = "") {
  const status = document.getElementById("admin-publicar-status") || document.getElementById("admin-login-status");
  if (!status) return;
...
function iniciarAdmin() {
  ...
  if (obterSenhaAdmin()) liberarPainelAdmin();
  else bloquearPainelAdmin();
  ...
  document.getElementById("btn-entrar-admin")?.addEventListener("click", () => {
    ...
    liberarPainelAdmin();
    mostrarStatusAdmin("Painel liberado. A senha será conferida pela Vercel na hora de publicar.", "sucesso");
  });
